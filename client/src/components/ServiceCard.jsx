  ...
  return (
    <div className="bg-theme-card rounded-2xl border border-theme hover:border-theme-accent theme-card-hover shadow-theme-sm transition-all duration-300 overflow-hidden flex flex-col h-full group">
      <div className="p-4 sm:p-6 flex flex-col flex-grow">
        ...
        <h3 className="text-base sm:text-lg font-bold text-theme-primary group-hover:text-theme-accent transition-colors duration-300 mb-2 font-poppins">
          {name}
        </h3>

        <p className="text-xs sm:text-sm text-theme-muted line-clamp-2 mb-4 flex-grow">
          {description || 'Professional fabric care service tailored to prolong the lifespan of your apparel.'}
        </p>

        {/* Price + action */}
        <div className="border-t border-theme pt-3.5 sm:pt-4 flex items-center justify-between mt-auto">
          <div>
            <span className="text-xl sm:text-2xl font-black text-theme-primary">₹{pricePerUnit}</span>
            <span className="text-xs text-theme-muted"> / {unit}</span>
          </div>

          <div className="relative z-10 mr-12 sm:mr-0">
            {quantity > 0 ? (
              <div className="flex items-center bg-theme-elevated border border-theme rounded-xl p-1 sm:p-1.5">
                ...
                <span className="px-2.5 sm:px-4 font-bold text-theme-primary text-sm sm:text-base">{quantity}</span>
                <button
                  type="button"
                  onClick={onAdd}
                  className="p-2 sm:p-1 min-w-[36px] min-h-[36px] sm:min-w-0 sm:min-h-0 flex items-center justify-center text-theme-muted hover:text-theme-primary hover:bg-theme-surface rounded-md transition-colors"
              ...
              <button
                type="button"
                onClick={onAdd}
                className="flex items-center space-x-1.5 px-3 sm:px-4 py-2 sm:py-2.5 bg-theme-elevated hover:bg-theme-accent hover:text-white border border-theme text-theme-primary rounded-xl text-xs sm:text-sm font-semibold transition-all duration-200 theme-btn-hover"
              >
